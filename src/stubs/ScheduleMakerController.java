...
public class ScheduleMakerController {
    ...
    public Collection<Schedule> createSchedule(ScheduleOptions schOpt) {
        ...
        //get courses
        // This class checks the ScheduleOptions class which its information
        // is derived from the createschedule page, this logic checks for empty fields.
        // If its not empty then insert into the courses String collection
...
//        courses.add(schOpt.getCourse6());
        ...
        // This checks if any courses were added, if not just return an empty schedule
        if (courses.size() <= 0)
            return schedules;
 
        //get campus, if all then insert both, else insert from schedule option 
